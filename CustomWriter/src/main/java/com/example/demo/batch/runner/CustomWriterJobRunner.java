...
@Component
public class CustomWriterJobRunner 
{
	private static final Logger logger = LoggerFactory.getLogger(CustomWriterJobRunner.class);

	@Autowired
	JobLauncher simpleJobLauncher;

	@Autowired
	Job defaultReaderWriter;

	@Async
	public void runBatchJob() 
	{
		JobParametersBuilder jobParametersBuilder = new JobParametersBuilder();
		jobParametersBuilder.addString(Constants.FILE_NAME_CONTEXT_KEY, "OneLakh.csv");
		jobParametersBuilder.addDate("date", new Date(), true);
		runJob(defaultReaderWriter, jobParametersBuilder.toJobParameters());
	}
